...
class controllerClass {
    ...
    public function create() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $rsp = modelClass::putNewUser($_POST['txtcod'], $_POST['txtdesc'], $_POST['txttel'], $_POST['txtdir'], $_POST['txtciu']);
            if ($rsp === true) {
                ...
                if (count($certificate) > 0) {
                    ...
                    if (is_array($data)) {
                        if (count($data) > 0) {
                            ...
                            $args['txttel'] = $data[0]['telefono'];
                            $args['txtdir'] = $data[0]['dir'];
                            $args['txtciu'] = $data[0]['cod_ciudad'];
                        }
                    } else {
                        $args['error'] = $data;
                        viewClass::renderHTML('error.php', $args);

...
class controllerClass {
    ...
    public function create() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $rsp = modelClass::putNewUser($_POST['txtcod'], $_POST['txtdesc'], $_POST['txtdep'],$_POST['txthab']);
            if ($rsp === true) {
    ...
    public function update() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
                ...
                if (count($certificate) > 0) {
                    ...
                    if (is_array($data)) {
                        if (count($data) > 0) {
                            ...
                            $args['txtdesc'] = $data[0]['nom_ciudad'];
                            $args['txtdep'] = $data[0]['cod_depto'];
                            $args['txthab'] = $data[0]['habitantes'];
            ...
            $data['nom_ciudad'] = $_POST['txtdesc'];
            $data['cod_depto'] = $_POST['txtdep'];
            $data['habitantes'] = $_POST['txthab'];

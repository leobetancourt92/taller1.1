...
class controllerClass {
    ...
    public function create() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $rsp = modelClass::putNewUser($_POST['txtid'], $_POST['txtapre'], $_POST['txttest']);
            if ($rsp === true) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
            ...
            if (is_array($certificate)) {
                if (count($certificate) > 0) {
                    ...
                    if (is_array($data)) {
                        if (count($data) > 0) {
                            $args['txtid'] = $data[0]['num_ficha'];
                            $args['txtapre'] = $data[0]['id_aprendiz'];
                            $args['txttest'] = $data[0]['estado'];
            ...
            $data['num_ficha'] = $_POST['txtid'];
            $data['estado'] = $_POST['txttest'];

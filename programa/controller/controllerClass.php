...
class controllerClass {
    ...
    public function create() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $rsp = modelClass::putNewUser($_POST['txtcod'], $_POST['txtdesc'],$_POST['txtdesc']);
            if ($rsp === true) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
            ...
            if (is_array($certificate)) {
                if (count($certificate) > 0) {
                    ...
                    if (is_array($data)) {
                        if (count($data) > 0) {
                            $args['txtcod'] = $data[0]['cod_pro'];
                            $args['txtdesc'] = $data[0]['desc_prog'];
                           $args['txtestd'] = $data[0]['estado'];
